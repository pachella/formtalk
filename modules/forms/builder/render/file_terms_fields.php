<?php
/**
 * Renderização de campos de arquivo e termos
 * Tipos: file, terms
 */

if (in_array($field['type'], ['file', 'terms'])): 
    ...
    $typeLabel = $typeLabels[$field['type']] ?? ucfirst($field['type']);
    $field_rendered = true;
?>
            ...
            <div class="flex items-start gap-3 flex-1">
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <?php endif; ?>
                        <?php if ($field['type'] === 'file' && PlanService::isFree()): ?>
                    ...
                    <?php endif; ?>
                    <p class="text-xs text-gray-500 dark:text-zinc-400">Tipo: <?= $typeLabel ?></p>
                    
                    <?php if ($field['type'] === 'file' && !empty($config['types'])): ?>
                        <p class="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                            Tipos permitidos: <?= htmlspecialchars($config['types']) ?>
                        </p>
                    <?php endif; ?>
                    
                    <?php if ($field['type'] === 'terms' && !empty($config)): ?>
                        <?php 
                        $termsModes = [
                            'inline' => 'Texto Inline (modal)',
                            'pdf' => 'PDF Anexado',
                            'link' => 'Link Externo'
                        ];
                        $termsMode = $termsModes[$config['mode'] ?? 'inline'] ?? 'Não definido';
                        ?>
                        <p class="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                            Modo: <?= $termsMode ?>
                        </p>
                        <?php if (($config['mode'] ?? 'inline') === 'inline' && !empty($config['text'])): ?>
                            <p class="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                                Texto: <?= mb_substr(strip_tags($config['text']), 0, 50) ?>...
                            </p>
                        <?php endif; ?>
                    <?php endif; ?>
